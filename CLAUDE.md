...
- **Silver / Gold sections** (collapsible, **default closed** — a deliberate flip from earlier open-by-default), each containing that metal's `PaperLeveragePanel` (live spot badge from `/api/prices/db`, leverage from `/api/silver|gold/db/leverage` — `open interest × contract size ÷ registered inventory`, 5,000oz silver contracts / 100oz gold), `SignalBanner` (current percentile + classification, both windows), and `SignalTrackRecord` (historical +4wk/+8wk hit-rate description, explicitly not prediction, thin samples flagged).
...
Both render diamonds (event-type colored via `CATCOR_EVENT_COLORS`); hovering a point in either chart doubles the size of that same event's point in the other (shared `hoveredEventId` state). Event-type legend folds in each type's next-scheduled date (no separate "Next Scheduled Catalysts" list).
...
## Tab: Research (CATCOR Iteration 2 — Research Pane)

**Frontend**: `frontend/src/research_panel.jsx`. **Backend**: `backend/catcor_research.py`, `backend/prompts/*.py`.

### Research: what it does

A place to work a single claim by hand: paste it, chat about it, eventually (not yet built) log a read and promote it to a tracked catalyst or dismiss it as noise. Minimal chat UI: first submit creates a `research_sessions` row, each send is one turn.

**Currently a deliberately trivial plumbing validator, not a real research feature yet.** Every turn goes through `prompts/word_count_v1.py`, a trivial word-counting persona used to validate chat plumbing (session/message persistence, backend routing, frontend rendering) independent of any real evidence-gathering. An earlier 2-call parser+analyst pipeline (`prompts/parser_v1.py` + `prompts/analyst_v1.py` — decompose a claim, fetch AV's own CoT/inventory/money-supply/market-balance data via read-only tools, then contextualize) is built and importable but **not currently wired into `send_message`**. The tool functions themselves (`_tool_get_cot_positioning`, `_tool_get_comex_inventory`, `_tool_get_money_supply`, `_tool_get_market_balance`, plus `dump_all_evidence`) still exist and back `GET /api/catcor/research/evidence/db` (a zero-cost, no-LLM-call dump of exactly what data the tools would see, for debugging).

### Model backend layering

`call_anthropic(system_prompt, messages, model)` / `call_forge(same signature)` — one function per vendor, identical signature and return shape (`{"final_text": str}`), pure text-in/text-out, no concept of tools at this layer. `call_ai(backend, ...)` is a thin dispatcher between them.

- **Anthropic**: raw httpx POST to `https://api.anthropic.com/v1/messages` (no SDK, same convention as every other outbound integration in this codebase), model `claude-haiku-4-5-20251001` — Haiku specifically, not Sonnet, a deliberate cost choice since this loop is exercised interactively/repeatedly during testing (confirm the exact model string against docs.claude.com if it ever needs correcting). Requires `ANTHROPIC_API_KEY`; has no mock mode — a `mock_key` passed to `call_anthropic` raises loudly rather than silently ignoring it, since this is real, billed traffic.
- **amp-forge** (`call_forge`): one POST to a local Ollama-backed LAN service's `/chat/stream` (`FORGE_URL`, default `http://amp-forge:8001/chat/stream`; see `forge-spec.md` in the amp-dev repo for the stateless/system-prompt contract), model `qwen3:8b`, accumulates SSE `token` events. Supports a `mock_key` to skip Ollama and stream back a fixed fixture instead, for testing AV's own data flow independent of live model quality.
- **`DEFAULT_BACKEND`**, read from `AI_BACKEND` env var, **defaults to `"forge"`** — Anthropic is an explicit opt-in for real (billed) compute, forge is the cheap local test box default. `main.py`'s startup check only requires `ANTHROPIC_API_KEY` when `AI_BACKEND=anthropic`.

### Research: data flow

`POST /api/catcor/research/sessions` → `catcor_research.create_session` (persists session row only, does NOT seed `research_messages` with the claim text) → immediately followed by a `send_message` call to get the claim into conversation history and produce the first reply (doing the seeding in both places would duplicate the first turn). `send_message`: persists the user turn immediately (never lost even if the model call fails) → resends history as plain text only (`_history_to_messages` collapses a stored assistant JSON envelope to just `final_text` — Claude/the model retains its own prior conclusions on resend, not a literal replay of which tool produced which number) → one `call_ai` → persists the assistant turn **only if the call fully resolves** (a network error or non-2xx leaves no assistant row, so no half-written replies land in `research_messages`).

Routes: `POST /api/catcor/research/sessions`, `GET /api/catcor/research/sessions/db`, `GET /api/catcor/research/sessions/{id}/db`, `POST /api/catcor/research/sessions/{id}/messages`, `GET /api/catcor/research/evidence/db`.

Tables: `research_sessions` (session_id PK, claim_text, source_url, status [active|promoted|dismissed — always `"active"` today, nothing sets the other two], user_read [bullish|bearish|neutral — not currently settable, no `/read` route], created_at, updated_at). `research_messages` (append-only, id/session_id/role/content/created_at — same never-edit convention as `cot_silver`/`cot_gold`). `research_log` (session_id, claim_text, source_url, user_read, dismissed_at, validation_status — **table exists in the schema but nothing writes to it yet**; the "log as noise" disposition is Deliverable 3, not built). `event_calendar` also carries `research_session_id` (backlink from a promoted catalyst to its originating session, NULL for government-seeded events) and a `direction` field (expected bullish/bearish direction of a promoted catalyst) — both added for this feature, both currently unused since promotion isn't wired up.

### Research: known gaps

- 📌 Deliverable 3 (promote to `event_calendar` / dismiss to `research_log`, `user_read` capture) not implemented — sessions can only be created and chatted in.
- 📌 The 2-call parser+analyst pipeline (real evidence-gathering via tools) is built but not wired into `send_message` — current chat is plumbing-only, doesn't actually reason over AV's data yet.
...
backend/
  ...
  prompts/                Research tab's model personas: word_count_v1.py (active), parser_v1.py + analyst_v1.py (built, not wired in)
...
frontend/
  src/App.jsx                 Top-level composition, owns activeSection tab state (SECTIONS: cot, moneySupply, inventory, catcor, research, data). All six sections mounted unconditionally with a section-hidden class toggled by activeSection — never conditionally rendered/unmounted, so switching tabs never refires mount-time fetches/listeners.
